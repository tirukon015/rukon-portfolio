...
| `GITHUB_TOKEN` | Homepage contribution calendar | **Required for the calendar.** A personal access token for the account the calendar belongs to; `read:user` (classic) or a fine-grained token with no repository permissions is enough, because only the contribution calendar is queried, via `viewer`. Private activity is counted in the totals when the GitHub profile setting to show it is on; no repository or commit data is ever requested. Read server-side only, in `src/lib/github/contributions.ts`, and cached for an hour. Without it the panel shows an unavailable state rather than any substitute data. |
